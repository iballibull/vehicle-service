...
const login = async (request) => {
  const input = validate(authSchema.login, request);

  const dealer = await authRepository.findUserByUsername(input?.username);

  if (!dealer) {
    throw new ResponseError(401, AUTHENTICATION_ERROR, { credentials: ['Username or password wrong'] });
  }

  const match = await bcrypt.compare(input?.password, dealer?.password);
